    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            message = render_to_string('accounts/account_verification_email.html', {
                ...
                # token generator is the library it has the make token and check token functions we'ere pass the user itself inside this make token function to create a token for this particular user
            })
        ...
        if user is not None:
            ...
            try:
                ...
                if 'next' in params:
                    ...
                    return redirect(
                        nextPage)  # if i want to do checkout and i wasnt login //after i logedin it takes me to the page after checkout page
            except:
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user,
                                                                token):  # we want to take the user from the token
        user.is_active = True
        user.save()
...
def edit_profile(request):
    ...
    if request.method == 'POST':
        user_form = UserForm(request.POST,
                             instance=request.user)  # we pass instance because we want to update the user profile not to create new one
...
def change_password(request):
    if request.method == 'POST':
        ...
        user = Account.objects.get(
            username__exact=request.user.username)  # user.username we have this inside the account model/ixact is case sensitive
        if new_password == confirm_password:
            success = user.check_password(current_password)  # check the old password if its right
            if success:
                user.set_password(
                    new_password)  # set_password its builled in function it takes the password and hash it
                user.save()
                # auth.logout(request)
...
def order_detail(request, order_id):
    order_detail = OrderProduct.objects.filter(
        order__order_number=order_id)  # order__order_number this from app orders model orderproduct insdie it we have order and its foreignkey with order model inside order model we have order_number
