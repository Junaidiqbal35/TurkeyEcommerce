...
def store(request,category_slug=None):
    ...
    if category_slug!=None:
      ...
      products = Product.objects.filter(maincategory=categories, is_available=True).order_by('id')
      paginator= Paginator(products,20)#6 is number of the products we want to show in each page  
...
def substore(request,subcategory_slug=None,maincategory_slug=None):
    # we bringing the slug here from urls file so nowe we have to accept the slug 
    subcategories = None
    products=None
    maincategories=None
    if subcategory_slug!=None :
      ...
      #subcategories = SubCategory.objects.filter(main_category=categories)#bring us the subcategory
      subcategories = SubCategory.objects.filter(slug=subcategory_slug)
      maincategories = Category.objects.filter(slug=maincategory_slug)

      #bring to us all the product of this category
      products = Product.objects.filter(subcategory__in=subcategories,maincategory__in=maincategories, is_available=True).order_by('id')
      paginator= Paginator(products,5)#6 is number of the products we want to show in each page  
      page= request.GET.get('page')
      paged_products=paginator.get_page(page)#the 6 product we get stored in paged_products and the (paged_products)pass in the template  
      product_count = products.count()
...
def product_detail(request, maincategory_slug, product_slug):
  ...
  try:
      single_product = Product.objects.get(maincategory__slug=maincategory_slug, slug=product_slug)
...
def subproduct_detail(request, category_slug, product_slug):
      # to pick a product from category first we should get this category and from this category we need to access to the slug to this category 
      # the category is in app.store.models.class product 
      # the slug of this category in  app.category.models.class category
  try:
        single_product = Product.objects.get(subcategory__slug=category_slug, slug=product_slug)
        in_cart = CartItem.objects.filter(cart__cart_id=_cart_id(request), product=single_product).exists()
  except Exception as e:
      raise e
